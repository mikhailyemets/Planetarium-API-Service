...
def get_admin_token():
    client = APIClient()
    response = client.post(
        "/api/user/token/", {"email": "admin@example.com", "password": "adminpassword"}
    )
    return response.data["access"]
...
class AstronomyShowTestCase(TestCase):
    ...
    def test_get_astronomy_show_list(self):
        AstronomyShow.objects.create(
            title="Galactic Adventure", description="Exploring galaxies far, far away."
        )
        response = self.client.get(self.list_url, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
...
class PlanetariumDomeTestCase(TestCase):
    ...
    def test_get_planetarium_dome_list(self):
        PlanetariumDome.objects.create(
            name="Orion Dome", rows=25, seats_in_row=30, price_per_seat="20.00"
        )
        response = self.client.get(self.list_url, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
    ...
    def test_update_planetarium_dome(self):
        dome = PlanetariumDome.objects.create(
            name="Old Name", rows=15, seats_in_row=20, price_per_seat="10.00"
        )
    ...
    def test_delete_planetarium_dome(self):
        dome = PlanetariumDome.objects.create(
            name="Delete Dome", rows=10, seats_in_row=20, price_per_seat="5.00"
        )
        url = reverse("planetarium:planetariumdome-detail", kwargs={"pk": dome.id})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
...
class ShowSessionTestCase(TestCase):
    def setUp(self):
        ...
        self.astronomy_show = AstronomyShow.objects.create(
            title="Test Show", description="A test show description."
        )
        self.astronomy_show.theme.add(self.show_theme)

        self.planetarium_dome = PlanetariumDome.objects.create(
            name="Test Dome", rows=10, seats_in_row=20, price_per_seat=Decimal("15.50")
        )
    ...
    def test_create_show_session_with_valid_payload(self):
        ...
        self.assertEqual(
            show_session.show_time, datetime(2023, 10, 15, 18, 0, tzinfo=timezone.utc)
        )
    ...
    def test_get_show_session_list(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]["astronomy_show"], self.astronomy_show.title)
        self.assertEqual(
            response.data[0]["planetarium_dome"], self.planetarium_dome.name
        )
    ...
    def test_update_show_session(self):
        ...
        url = reverse("planetarium:showsession-detail", kwargs={"pk": show_session.id})
        update_payload = {
            "astronomy_show": self.astronomy_show.id,
            "planetarium_dome": self.planetarium_dome.id,
        ...
        self.assertEqual(
            show_session.show_time, datetime(2023, 10, 16, 19, 0, tzinfo=timezone.utc)
        )
...
class ReservationAPITestCase(TestCase):
    ...
    def test_create_reservation_with_valid_payload(self):
        response = self.client.post(
            reverse("planetarium:reservation-list"), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_delete_reservation(self):
        ...
        url = reverse("planetarium:reservation-detail", kwargs={"pk": reservation.id})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
...
class TicketModelTestCase(TestCase):
    ...
    def test_valid_ticket(self):
        ticket = Ticket(
            row=3, seat=5, show_session=self.session, reservation=self.reservation
        )
        ticket.clean()
        ticket.save()
    ...
    def test_invalid_ticket_row(self):
        with self.assertRaises(ValidationError):
            ticket = Ticket(
                row=0, seat=5, show_session=self.session, reservation=self.reservation
            )
            ticket.clean()
    ...
    def test_invalid_ticket_seat(self):
        with self.assertRaises(ValidationError):
            ticket = Ticket(
                row=3, seat=15, show_session=self.session, reservation=self.reservation
            )
            ticket.clean()
